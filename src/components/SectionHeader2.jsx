      ...
      <div className="flex flex-wrap gap-1 md:gap-2 ">
        {displayCategories.map((c, index) => (
          <button
            ...
            className={`px-3 py-1 text-xs md:text-sm rounded-full cursor-pointer transition-all duration-300 ease-in-out
              ${
                selected === c.id
        ...
        {categories.length > minDisplay && (
          <button
            ...
            className="cursor-pointer px-3 py-1 text-xs md:text-sm rounded-full bg-white hover:bg-gray-200"
          >
            {showAll ? "Ẩn bớt" : "Thêm"}
          </button>
        )}
